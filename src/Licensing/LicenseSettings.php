<?php
/**
 * FluentCart license settings admin page.
 *
 * Renders the license activation/deactivation UI and handles
 * AJAX requests for license management.
 *
...
final class LicenseSettings {
     ...
     *
     * @param FluentLicensing|null  $licensing Licensing instance.
     * @param array<string, mixed>  $config    Configuration.
     * @return self
     */
    public function register(?FluentLicensing $licensing = null, array $config = []): self {
        if (self::$instance) {
    ...
    /**
     * Handle license status AJAX request.
     */
    public function handleLicenseStatusAjax(): void {
        ...
        if (is_wp_error($status)) {
            wp_send_json(['error_notice' => $status->get_error_message()]);
        }

        $message = '';
        if (!empty($status['is_expired'])) {
            $message = '<p>Your license has expired.</p>';
            if (!empty($status['renewal_url'])) {
                $message .= '<p><a href="' . esc_url($status['renewal_url']) . '" target="_blank" class="button button-primary">Renew License</a></p>';
            }
        } elseif (!empty($status['error_type']) && $status['error_type'] === 'disabled') {
            $message = $status['message'] ?? '<p>Your license has been disabled. Please contact support.</p>';
        }
    ...
    /**
     * Create the menu page based on type.
     */
    ...
    public function renderLicensingContent(): void {
            ...
            <div id="fct_license_body" class="fct_licensing_body">
                <?php if ($licenseStatus['status'] === 'valid'): ?>
                    ...
                    <?php if (($licenseStatus['expires'] ?? '') !== 'lifetime'): ?>
                        <p><strong>Expires On:</strong> <?php echo esc_html($licenseStatus['expires'] ?: 'Never'); ?></p>
                    <?php else: ?>
                ...
                <?php else: ?>
                    ...
                    <?php if ($purchaseUrl): ?>
                        <div class="fct_purchase_wrap">
                            <p>
                                Don't have a license? <a rel="noopener" href="<?php echo esc_url($purchaseUrl); ?>" target="_blank">
                                    Purchase License
                                </a>
                            </p>
                        </div>
        ...
        <script type="text/javascript">
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function setError(errorMessage, isHtml) {
                    isHtml = isHtml || false;
                ...
                function sendAjaxRequest(action, data) {
                    ...
                    data.action = '<?php echo esc_js($slug); ?>_license_' + action;
                    data._nonce = _nonce;
                ...
                sendAjaxRequest('status', {})
                    .then(function (response) {
                        if (response.error_notice) setError(response.error_notice, true);
                    });
                <?php endif; ?>
        ...
        <style>
            ...
            .fct_loader_item svg { fill: #686a6b; width: 40px; }
            .fct_error_notice {
                color: #ff4e16;
                margin-top: 20px;
                font-size: 15px;
            }
            .fct_error_notice p { font-size: 15px; }
            .fct_purchase_wrap {
                margin-top: 20px;
                display: block;
                overflow: hidden;
            ...
            .fct_licensing_body { padding: 30px 20px; }
